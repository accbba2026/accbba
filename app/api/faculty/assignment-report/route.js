// app/api/faculty/assignment-report/route.js
  ...
  try {
    ...
    if (!currentUser || (currentUser.role !== 'faculty' && currentUser.role !== 'admin')) {
      ...
      await Log.create({
        ...
        details: JSON.stringify({
          action: "Unauthorized Report Access Attempt",
          user: currentUser ? {
            id: currentUser._id,
    ...
    if (!semester) {
      await Log.create({
        ...
        resourceType: "report",
        details: JSON.stringify({
          action: "Report Generation Failed - Missing Semester",
          user: {
            id: currentUser._id,
            name: currentUser.name,
            collegeId: currentUser.collegeId,
    ...
    await Log.create({
      ...
      resourceType: "report",
      details: JSON.stringify({
        action: "Assignment Report Generation Attempt",
        user: {
          id: currentUser._id,
          name: currentUser.name,
          collegeId: currentUser.collegeId,
    ...
    const originalAssignmentCount = assignments.length;
    ...
    // Calculate per-student submission statistics
    ...
    // Calculate per-assignment submission statistics
    ...
    try {
      ...
      await Log.create({
        ...
        resourceType: "report",
        details: JSON.stringify({
          action: "Assignment Report Generation Failed",
          user: currentUser ? {
            id: currentUser._id,
            name: currentUser.name,
            collegeId: currentUser.collegeId,
